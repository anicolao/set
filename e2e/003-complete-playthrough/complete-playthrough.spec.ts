...
async function getBoardCards(page: Page): Promise<Card[]> {
    const locators = await page.locator('.card-wrapper').all();
    const cards: Card[] = [];
...
test.describe('Complete Playthrough', () => {
    test.setTimeout(30000); // 30s is enough with clock manipulation
    ...
    test('play a full game with 2 players', async ({ page }, testInfo) => {
        ...
        while (gameActive) {
            ...
            if (setIds) {
                ...
                await expect(page.locator(`text=${playerName} called SET!`).first()).toBeVisible();
                ...
                for (let i = 0; i < setIds.length; i++) {
                    ...
                    // Click inner .card to ensure we hit the element with the click handler
                    await cardLocator.locator('.card').click();
                    ...
                    if (i < 2) {
                        await expect(cardLocator.locator('.card')).toHaveClass(/selected/);
                    }
                }

                // Fast forward through the 1.5s animation
                ...
                // Check Deal More is BLOCKED when a set exists - REMOVED because main branch allows dealing
                ...
                if (newCount > cards.length) {
                    ...
                    if (await page.locator('text=Game Over!').first().isVisible()) {
                        ...
                        // Recheck message
                        const msg = await page.locator('.controls').first().textContent();
